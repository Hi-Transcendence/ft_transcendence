...
export class GameManager {
    ...
    async addNewGame(data: any, server: Server) {
        ...
        await this.userRepository.update({ nickname: data.firstNick }, { channel_id: game.channelId, status: 'gamming' });
        await this.userRepository.update({ nickname: data.secondNick }, { channel_id: game.channelId, status: 'gamming' });

        game.startGame().then(async (data) => {
            await this.userRepository.update({ nickname: game.firstPlayer.nickName }, { status: 'online' });
            await this.userRepository.update({ nickname: game.secondPlayer.nickName }, { status: 'online' });
        });
    ...
    closeGame(channelId: string) {
        ...
        this.gameChannelList = this.games.filter(gameChannel => gameChannel.channelId !== channelId);
    }
